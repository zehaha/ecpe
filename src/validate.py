...
kernel = """
	...
	__kernel void validate(__global int* result_vector, __global int* result_matrix, __global int* candapps, 
		...
		int valid = 1, temp_result_matrix[MAT_SIZE], grpid = get_group_id(0), localid = get_local_id(0);
		int first_index = 0, some = 0;
		...
		if(numapps[grpid] > (int) (localid / ROW_SIZE)) {
			...
			for(int i = 0; i < INDICES_LENGTH; i ++) {
				for(int j = 0; j < COL_SIZE; j ++) temp_result_matrix[i * COL_SIZE + j] *= candapps[first_index + (localid * COL_SIZE) + j];
				int sum = 0;
				for(int j = 0; j < COL_SIZE; j ++) sum += temp_result_matrix[i * COL_SIZE + j];
				if(sum > config[indices[i]]) {
					valid = 0;
					some = sum;
					break;
				}
			...
			//result_vector[(int)(first_index / COL_SIZE) + localid] = some;
		}
	} 
	"""
...
kernel = indices_length + mat_size + column_size + row_size + kernel
...
# execute the kernel
program.validate(queue, (max_numapps*num_configs, ), (max_numapps, ), result_vector_buf, result_buf, candapps_buf, config_vectors_buf, indices_buf, numapps_buf)
